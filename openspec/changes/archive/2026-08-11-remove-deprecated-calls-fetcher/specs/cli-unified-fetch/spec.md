...
### Requirement: Legacy CallsFetcher class functionality
**Reason**: Deprecated and replaced by `OpenCallsFetcher` and `ClosedCallsFetcher` which handle JSONL.GZ format and align with data organization refactor.
**Migration**: Users of `CallsFetcher` should migrate to using the two new fetchers directly, or rely on the CLI command `cordis-data fetch-calls` for simplified orchestration.
