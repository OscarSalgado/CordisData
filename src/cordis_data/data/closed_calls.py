"""EU research funding calls fetcher for closed opportunities."""
...
class ClosedCallsFetcher:
    ...
    def __init__(self, sedia_client: Optional[SediaClient] = None) -> None:
        """Initialize ClosedCallsFetcher with optional SEDIA client.
    ...
    def _build_query(self, until_date: Optional[str] = None) -> dict[str, Any]:
        """Build Elasticsearch query for closed calls.

        Args:
            until_date: ISO date string (YYYY-MM-DDTHH:MM:SS.SSSZ) or None
        ...
        must: list[dict[str, Any]] = [
            ...
            {"terms": {"type": ["1", "2"]}},
            # Only closed (31094503)
            {"terms": {"status": ["31094503"]}}
        ]
        if until_date:
            must.append({"range": {"startDate": {"lte": until_date}}})
    ...
    def main(
        ...
        """Fetch closed calls from SEDIA and write to output file.
        ...
        if (
            not force
            and not is_stale(
                metadata.get("calls_closed_fetched_at"),
                metadata.get("calls_closed_freshness_ttl_days", 7),
            )
        ):
            print("Closed calls data is fresh. Skipping fetch.", flush=True)
            return

        # Filter: closed calls up to 3 months ago (don't include recent closures)
        cutoff_day = datetime.date.today() - datetime.timedelta(days=90)
        until_date = cutoff_day.strftime("%Y-%m-%dT23:59:59.999Z")
        print(
            f"Fetching closed calls with startDate <= {until_date} "
            "(excluding last 3 months)", flush=True,
        )

        query = self._build_query(until_date)
        ...
        print(f"\nTotal closed calls: {len(merged_calls)}")
        ...
        # Generate changelog for closed calls
        today_str = datetime.date.today().isoformat()
        changelog = generate_compact_changelog_calls(existing_calls, merged_by_id, 0)
        changelog_path = output_path.parent / "changelog" / "closed" / f"{today_str}.json"
